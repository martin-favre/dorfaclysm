...
class SnekPiece
{
    ...
    void render()
    {
        GraphicsManager::drawRect(Rect{mPos.x*mSize.x, mPos.y*mSize.y, mSize.x, mSize.y});
        if(mChild)
        {
            mChild->render();
        }
...
int main()
{
    ...
    while(true)
    {
        ...
        if(snek->atTarget())
        {
            ...
            }while((targetPos == snek->mSnekHead.mPos) || !map.getTile(targetPos)->isOpen());
        }
        GraphicsManager::setRenderDrawColor(SDL_Color{255, 0, 0, 255});
        GraphicsManager::drawRect(Rect{targetPos.x*snek->mSnekHead.mSize.x, targetPos.y*snek->mSnekHead.mSize.x, 5, 5});
